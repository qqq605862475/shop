...
class Links extends Base
{
    public function add()
    {
        ...
        if (request()->isPost()){
            ...
            //验证
            $validate=Loader::Validate('Links'); //获取表中的数据 Admin是类名 必须首字母大写
            ...
            //判断添加是否成功
            if($res){
                return $this->success('添加成功',url('Links/index'));//成功返回
            }
            else{
...
//        $data=Db::name('admin')->select();//返回结果是二维数组
        ...
        //分页
        $data=Db::name('links')->paginate(3);//返回结果是一个对象
        //把变量分配到模版
